...
- **引流词三禁（最高优先）**：出现「微信」「私信」「评论区」任一个，或它们的组合变体（加微信 / 私信我 / 评论区留言 / 扣在评论区），**一律判不可用、退回重写**。平台会自动判定引流并压推荐流量 —— 安全替代写法见 `ip-voice-playbook` §6.9
- 顺带查有没有留联系方式（号码、二维码、外部链接，以及「去主页」「看简介」这类引路话术）
